...
- 将 Sketch 的核心组件尺寸、间距、状态和响应式规则转换为 AI 可读参数表，并保留来源 UUID 和冲突记录。
...
这个入口会要求 AI 先读取黄金样例，再读取规则和校验清单。不要只上传单个 `SKILL.md`。

组件参数由 `data/sketch-component-contract.json` 统一生成。普通生成任务只需按 `knowledge/component-parameters/index.md` 读取本页相关参数，不必加载完整 Sketch JSON。
...
golden-demo/   当前最终 demo，作为稳定生成的黄金样例
templates/     静态 demo 生成模板
knowledge/     设计原则、页面模式、组件规则、结构化参数表、反例
data/          token、组件库、页面配方、业务语义、字段映射、Sketch 机器合同
scripts/       Sketch 提取与分发同步工具
...
1. 先读 `START-HERE-FOR-AI.md` 和 `golden-demo/`。
2. 按页面组件读取 `knowledge/component-parameters/` 中对应参数表。
3. 输出页面理解：页面类型、业务对象、使用角色、核心目标、字段、操作、状态和 TODO。
4. 输出结构规划：选择 Page Recipe、阅读顺序、页面区域和状态覆盖。
5. 输出组件映射：字段、操作、状态映射到监管平台组件和 Dorami 基础控件。
6. 生成页面 demo：优先复用 `templates/static-demo/` 和黄金样例结构。
7. 一致性自检：按 `compiler/golden-demo-checklist.md`、`compiler/validation-checklist.md` 和反例规则检查。
